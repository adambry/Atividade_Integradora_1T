...
dados <- read_csv("C:/Users/Flavio/Dropbox/My PC (LAPTOP-FVTATBSD)/Documents/Pós Insper Data Science/Projeto Integrador AEM I e CCD/df.csv", show_col_types = FALSE) %>% 
  dplyr::select(-comp_id, -curr_idx_liq, -log_sales)#dropando curr_idx_liq e log_sales
```
...
```{r}
result_arvores <- crossing(mtry = c(2, 4, 8, 12, 16, 20, 24),
...
```{r}
dados_pequena <- dados %>%
  filter(comp_size == 'Pequena') #incluir o nome da coluna que possui os dados de porte e colocar o nome do porte como pequena
dados_pequena <- subset(dados_pequena, select = -c(comp_size)) #incluir o nome da coluna que possui os dados de porte para excluí-la

dados_media <- dados %>%
  filter(comp_size == 'Média') #incluir o nome da coluna que possui os dados de porte e colocar o nome do porte como media
dados_media<- subset(dados_media, select = -c(comp_size)) #incluir o nome da coluna que possui os dados de porte para excluí-la

dados_grande <- dados %>%
  filter(comp_size == 'Grande') #incluir o nome da coluna que possui os dados de porte e colocar o nome do porte como grande
dados_grande <- subset(dados_grande, select = -c(comp_size)) #incluir o nome da coluna que possui os dados de porte para excluí-la
```


Criando os datasets de treino e teste para cada um dos portes:
```{r}
idx_pequena <- sample(nrow(dados_pequena), size = .80 * nrow(dados_pequena ), replace = FALSE)
idx_media <- sample(nrow(dados_media), size = .80 * nrow(dados_media), replace = FALSE)
idx_grande <- sample(nrow(dados_grande), size = .80 * nrow(dados_grande), replace = FALSE)

treino_pequena <- dados_pequena[idx_pequena,]
teste_pequena <- dados_pequena[-idx_pequena,]

treino_media <- dados_media[idx_media,]
teste_media  <- dados_media[-idx_media,]

treino_grande <- dados_grande[idx_grande,]
teste_grande  <- dados_grande[-idx_grande,]
...
```{r}
desempenho <- desempenho %>% 
  bind_rows(tibble(prob = prob_log_pequena, 
...
```{r}
prob_log_media <- predict(log_media, newdata = teste_media, type = "response")
```
...
```{r}
desempenho <- desempenho %>% 
  bind_rows(tibble(prob = prob_log_media, 
                   classes = teste_media$will_close, 
                   metodo = "logística medio porte")) 
```
...
```{r}
desempenho <- desempenho %>% 
  bind_rows( tibble(prob = prob_log_grande, 
                   classes = teste_grande$will_close, 
                   metodo = "logística grande porte")) 
```
