...
```{r}
vip(log_stepwise, mapping = aes(fill = Sign)) + labs(title = "Modelo de Regressão Logística com Stepwise")
```
...
```{r}
vip(log_sem_outliers, mapping = aes(fill = Sign)) + labs(title = "Modelo de Regressão Logística sem Outliers")
```
...
```{r}
X_treino_sem_outliers <- model.matrix(will_close ~ ., dados_sem_outliers)[idx,-1]
y_treino_sem_outliers <- dados_sem_outliers$will_close[idx]

X_teste_sem_outliers <- model.matrix(will_close ~ ., dados_sem_outliers)[-idx,-1]
y_teste_sem_outliers <- dados_sem_outliers$will_close[-idx]
```
...
```{r}
vip(ridge_sem_outliers, mapping = aes(fill = Sign)) + labs(title = "Rigde sem Outliers")
```
...
```{r}
vip(lasso_sem_outliers, mapping = aes(fill = Sign)) + labs(title = "LASSO sem Outliers")
```
